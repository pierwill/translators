{
	...
	"lastUpdated": "2018-02-15 03:05:42"
}
...
function catalogue(doc,url) {
  ...
  // TCP id (e.g., A20631)
  var tcpID = IDs.split(" ")[1].split(";")[0];

  // use permalink for item url
  ...
  // url for html
  var linkurl = "http://tei.it.ox.ac.uk/tcp/Texts-HTML/free/" + tcpID.slice(0,3) + "/" + tcpID + ".html";
...
function html(doc, url) {
  ...
  var tcpID = url.split("/")[7].split(".html")[0];
  var newURL = "http://ota.ox.ac.uk/id/" + tcpID;

  // use permalink for item url
  newItem.url = newURL;

  // On the html reading pages themselves,
  // we use the catalogue entries for metadata.
